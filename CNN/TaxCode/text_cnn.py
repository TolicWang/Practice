...
class TextCNN(object):
    def __init__(self,
                 ...
                 num_filters,
                 l2_reg_lambda):
        self.input_x = tf.placeholder(tf.float32, [None, sequence_length, embedding_dimension], name='x-input')
        self.input_y = tf.placeholder(tf.int64, [None], name='y-input')
        ...
        for filter_size in filter_sizes:
            with tf.name_scope('conv-maxpool-%s' % filter_size):
                filter_shape = [filter_size, embedding_dimension, 1, num_filters]
